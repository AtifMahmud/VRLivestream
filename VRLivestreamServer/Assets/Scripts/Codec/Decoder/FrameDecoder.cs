...
public class FrameDecoder {
    ...
    /// </exception>
    public List<byte[]> DecodeFrame(List<Packet> encoded_packets) {
        int success = 0;
        foreach (var packet in encoded_packets)
            CodecUtility.SendPacket(decoder_context_, packet);
        ...
        while (success == 0) {
            ...
            if (success == 0) {
                ...
                byte[] frame = rgb_frame_.CopyToManagedMem();
                // Unity needs a bottom-up texture instead of a top-up texture
                frames.Add(Codec.CodecUtility.GetFlippedArray(frame, yuv_frame_.Height(), frame.Length / yuv_frame_.Height()));
            }
